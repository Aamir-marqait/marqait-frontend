...
import { Download, Sparkles, RefreshCw, ChevronLeft, ChevronDown, Copy, Check } from "lucide-react";
...
const SocialMediaPostGenerator = () => {
    ...
    try {
      ...
      const generateHashtags = () => {
        const platformHashtags: Record<string, string[]> = {
          instagram: ['#instagram', '#insta', '#post', '#viral', '#trending', '#creative', '#brand', '#design', '#social', '#content'],
          facebook: ['#facebook', '#social', '#community', '#share', '#engage', '#brand', '#marketing', '#content'],
          twitter: ['#twitter', '#tweet', '#trending', '#viral', '#social', '#brand', '#content'],
          linkedin: ['#linkedin', '#professional', '#business', '#networking', '#career', '#industry', '#corporate', '#brand']
        };
        
        const baseHashtags = platformHashtags[formData.platform] || platformHashtags.instagram;
      ...
      const generateBestTime = () => {
        ...
        return bestTimes[formData.platform] || 'Tuesday - Thursday, 11 AM - 1 PM';
      };
      ...
      const generateDummyPost = () => {
        ...
        const dimensions = formData.contentType === "story" 
          ? { width: 400, height: 600 } 
          : { width: 500, height: 500 };

        // Create a simple SVG post as data URL
        const svgPost = `
          <svg width="${dimensions.width}" height="${dimensions.height}" xmlns="http://www.w3.org/2000/svg">
            ...
            <rect x="20" y="20" width="${dimensions.width - 40}" height="${dimensions.height - 40}" fill="white" opacity="0.95" rx="15"/>
            <text x="${dimensions.width/2}" y="80" font-family="Inter, Arial, sans-serif" font-size="24" font-weight="bold" text-anchor="middle" fill="${colors.bg}">SOCIAL MEDIA POST</text>
            <text x="${dimensions.width/2}" y="120" font-family="Inter, Arial, sans-serif" font-size="16" text-anchor="middle" fill="${colors.accent}">${formData.platform.toUpperCase()} ${formData.contentType.toUpperCase()}</text>
            <foreignObject x="40" y="150" width="${dimensions.width - 80}" height="${dimensions.height - 200}">
              <div xmlns="http://www.w3.org/1999/xhtml" style="font-family: Inter, Arial, sans-serif; font-size: 14px; color: #333; text-align: center; padding: 20px; line-height: 1.5;">
                ${formData.contentDescription.length > 100 
                  ? formData.contentDescription.substring(0, 100) + "..." 
                  : formData.contentDescription}
              </div>
            </foreignObject>
            <text x="${dimensions.width/2}" y="${dimensions.height - 30}" font-family="Inter, Arial, sans-serif" font-size="12" text-anchor="middle" fill="${colors.bg}" opacity="0.8">Generated with MARQAIT</text>
          </svg>
        `;

        return `data:image/svg+xml;base64,${btoa(svgPost)}`;
      };

      const postUrl = generateDummyPost();
  ...
  const handleDownload = (format: 'png' | 'svg') => {
    ...
    const fileName = `social_media_post_${formData.platform}_${Date.now()}`;
    
    if (format === 'svg') {
      // Convert the current SVG data URL to downloadable SVG
      if (generatedPost.imageUrl.startsWith('data:image/svg+xml;base64,')) {
        const svgData = atob(generatedPost.imageUrl.split(',')[1]);
      ...
      if (generatedPost.imageUrl.startsWith('data:image/svg+xml;base64,')) {
        atob(generatedPost.imageUrl.split(',')[1]);
        const img = new Image();
        img.onload = () => {
          const canvas = document.createElement('canvas');
          const ctx = canvas.getContext('2d');
          canvas.width = 800;
          canvas.height = formData.contentType === "story" ? 1200 : 800;
        ...
        img.src = generatedPost.imageUrl;
        return;
      } else {
          ...
          <>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-xl p-4 border border-[#E0E0E0] shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-inter font-semibold text-base text-[#8F00FF]">
                      Caption
                    </h3>
                    <button
                      onClick={() => handleCopy(generatedPost.caption, 'caption')}
                      className="inline-flex h-8 items-center gap-1 rounded-lg border border-[#8F00FF] bg-white px-2 font-inter font-medium text-xs text-[#8F00FF] transition hover:bg-[#faf5ff] cursor-pointer"
                    >
                      {copiedCaption ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                      {copiedCaption ? 'Copied!' : 'Copy'}
                    </button>
                  </div>
                  <p className="font-inter font-normal text-sm text-[#4B4B4B] leading-5 whitespace-pre-line">
                ...
                <div className="bg-white rounded-xl p-4 border border-[#E0E0E0] shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      onClick={() => handleCopy(generatedPost.hashtags, 'hashtags')}
                      className="inline-flex h-8 items-center gap-1 rounded-lg border border-[#8F00FF] bg-white px-2 font-inter font-medium text-xs text-[#8F00FF] transition hover:bg-[#faf5ff] cursor-pointer"
                    >
              ...
              <div className="bg-[#F5EDFF] rounded-xl p-6 border border-[#E0D3FA] flex flex-col justify-center">
                <h3 className="font-inter font-semibold text-lg text-[#8F00FF] mb-4">
                  🕰️ Best Time to Post
                </h3>
                <p className="font-inter font-semibold text-base text-[#4B4B4B] leading-6 mb-4">
                  {generatedPost.bestTime}
                </p>
                <p className="font-inter font-normal text-sm text-[#6B7280] leading-5">
                  Optimal posting time for {formData.platform.charAt(0).toUpperCase() + formData.platform.slice(1)} based on audience engagement patterns.
                </p>
              </div>
            </div>
          </>
